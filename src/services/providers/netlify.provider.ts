...
export const netlifyProvider: Provider = {
  ...
  async getUser(accessToken: string): Promise<ProviderUser> {
    const data = await netlifyFetch<NetlifyUser>(`${NETLIFY_API_BASE}/user`, accessToken);
  ...
  async listProjects(accessToken: string): Promise<ProviderProject[]> {
    const sites = await netlifyFetch<NetlifySite[]>(`${NETLIFY_API_BASE}/sites`, accessToken);

    return sites.map((site) => ({
      id: site.id,
      name: site.name,
      linkedRepo: site.build_settings?.repo_url
  ...
  async listEnvVars(
    ...
    logger.debug(
      { siteId, accountId: site.account_id, accountName: site.account_name },
      "Got site info"
    );
    ...
    const vars = await netlifyFetch<NetlifyEnvVar[]>(envUrl, accessToken);
    ...
    return (
      vars
        ...
        .filter((v) =>
          v.values?.some((val) => val.context === environment || val.context === "all")
        )
        .map((v) => {
          // Prefer specific context, fall back to 'all'
          const contextValue =
            v.values?.find((val) => val.context === environment) ||
  ...
  async setEnvVars(
    ...
    // First, get the site to retrieve the account_id (required for new API)
    logger.debug(
      { siteId, environment, varCount: Object.keys(vars).length },
      "Starting setEnvVars"
    );
    const site = await netlifyFetch<NetlifySite>(
      `${NETLIFY_API_BASE}/sites/${siteId}`,
    ...
    logger.debug(
      { siteId, accountId, accountName: site.account_name },
      "Got site info for setEnvVars"
    );
    ...
    const existingVars = await netlifyFetch<NetlifyEnvVar[]>(envUrl, accessToken);
    const existingKeys = new Set(existingVars.map((v) => v.key));
    logger.debug(
      { existingCount: existingVars.length, existingKeys: Array.from(existingKeys) },
      "Got existing env vars"
    );
    ...
    logger.debug(
      { newCount: newVarsEntries.length, updateCount: existingVarsEntries.length },
      "Separated vars for create vs update"
    );
    ...
    if (newVarsEntries.length > 0) {
      ...
      logger.debug(
        { createUrl, keys: newVarsEntries.map(([k]) => k) },
        "Bulk creating new env vars"
      );

      try {
        await netlifyFetch(createUrl, accessToken, {
          method: "POST",
    ...
    for (const [key, value] of existingVarsEntries) {
      ...
      try {
        await netlifyFetch(updateUrl, accessToken, {
          method: "PATCH",
          body: JSON.stringify({
            context: environment,
            value,
  ...
  async deleteEnvVar(
    ...
    await netlifyFetch(deleteUrl, accessToken, { method: "DELETE" });
