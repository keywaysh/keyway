...
async function railwayGraphQL<T>(
  ...
  try {
    ...
    if (data.errors && data.errors.length > 0) {
      ...
      throw new RailwayProviderError(error.message, error.extensions?.code, response.status);
    }
...
async function triggerRailwayRedeploy(
  ...
  // Trigger redeploy
  await railwayGraphQL(MUTATIONS.redeployDeployment, { id: latestDeployment.id }, accessToken);
}
          ...
          logger.warn(
            {
              serviceId,
              environmentId,
              error: redeployError instanceof Error ? redeployError.message : "Unknown error",
            },
