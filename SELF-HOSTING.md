...
This guide covers deploying Keyway on your own infrastructure using Docker Compose.
...
By default, all URLs are derived from `DOMAIN`:
...
Controls plan limits (free vs pro vs team). **Disabled by default.**

```bash
BILLING_ENABLED=false  # Default: all users get unlimited access
```
...
Caddy needs ports 80 and 443 open for the ACME challenge. Check:
```bash
...
The dashboard derives the cookie domain from the browser hostname. If using a non-standard subdomain setup, verify that the API and dashboard share a common parent domain.
