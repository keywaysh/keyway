...
-- Stripe customer never paid → personal accounts go to 'free'; org plans
-- granted manually (no Stripe, no trial) are deliberately kept.

-- 1) Remap data while 'pro' still exists in the enum
UPDATE "organizations" SET "plan" = 'team' WHERE "plan" = 'pro' AND "stripe_customer_id" IS NOT NULL;
UPDATE "organizations" SET "plan" = 'free' WHERE "plan" = 'pro';
UPDATE "users" SET "plan" = 'free' WHERE "plan" != 'free' AND "stripe_customer_id" IS NULL;
...
ALTER TYPE "user_plan" RENAME TO "user_plan_old";
CREATE TYPE "user_plan" AS ENUM ('free', 'team', 'business');
...
ALTER TABLE "organizations" ALTER COLUMN "plan" TYPE "user_plan" USING "plan"::text::"user_plan";
