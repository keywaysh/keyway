...
organizations. Two problems surfaced. Counting private repos as the scaling
lever penalizes multi-repo architectures while under-charging monoliths, and
selling the same paid tiers to both personal accounts and organizations made
the payment workflow ambiguous (an owner could pay for themselves while
intending to pay for their org).
...
2. **Paid tiers are flat per organization and differ by features, not quotas.**
   ...
   Business additionally unlocks Exposure and SSO. Members/collaborators are
   never capped on any tier (access mirrors GitHub: repo access = secret
   access). The Free tier allows 10 private repos (up from 1); public repos
   remain unlimited everywhere.
...
4. **Lapsed trials are normalized.** Orgs whose trial expired unconverted (no
   ...
   Org plans granted manually (no Stripe customer, no trial) are deliberately
   kept.
...
- Webhooks for legacy `pro_*`-priced subscriptions no longer resolve to a plan
  and are dropped with a warning; acceptable because no such live subscriptions
  exist, and the personal-checkout retirement handles the remainder.
- The private-vault read-only (soft-limit) ordering is now deterministic:
  `created_at` ties break on `id`.
